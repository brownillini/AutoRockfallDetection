%% Basic Fall Detection and warning
% This example is based off of MatLab's provided Motion-Based Multiple
% Object Tracking example (which handles background segmentation, blob
% detection, and labeling of moving objects with a numeric ID representing
% order of detection.
%
% I have updated the example with more sensitive object detection settings,
% internal per-object directionality and speed settings, indicator of
% whether the object could be classified as "falling" (based on direction
% and speed), frame duration of fall, color-based falling/nonfalling label,
% and a warning signal (to console) indicating a detected fall.
...
fallDurationForWarning = 5; % Total frames of "falling" before warning sent
...
while hasFrame(obj.reader)
    ...
    pause(0.067); % Slow playback to roughly 15FPS
end
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects()
        ...
        obj.reader = VideoReader('C:\AppDev\GCEGitHub\GCE-Thermal\Background Segmentation\Thermal Rockfall Examples\Examples\Low contrast slide.mp4');
...
% visible count by 1. Finally, the function sets the invisible count to 0. 
    ...
    function updateAssignedTracks()
        ...
        for i = 1:numAssignedTracks
            ...
            % Update center, previous center, and directionality.
            tracks(trackIdx).previousCenter = tracks(trackIdx).currentCenter;
            newBaseCenter = centroid;
            newBaseCenter(2) = centroid(2) + bbox(4)/2;
            tracks(trackIdx).currentCenter = newBaseCenter;
            [tracks(trackIdx).direction, tracks(trackIdx).magnitude, ...
                tracks(trackIdx).falling] = ...
                findDirection(tracks(trackIdx).currentCenter, ...
                tracks(trackIdx).previousCenter);

            % Update track's total fall duration.
            if tracks(trackIdx).falling
                tracks(trackIdx).fallDuration = ...
                    tracks(trackIdx).fallDuration + 1;
                % Send warning if falling and duration exceeds threshold.
                if tracks(trackIdx).fallDuration > fallDurationForWarning
                    disp(strcat("Rockfall detected at location ", ...
                        num2str(centroid)))
                end
            end
...
% Generate a directional vector of the current vs previous location of the
% track centroid, normalize, then find the angle of motion (in degrees)
    function [direction,magnitude,fall] = findDirection(current,previous)
        vector = current - previous;
        %disp(vector)
        magnitude = sqrt((vector(1))^2 + (vector(2))^2);
        %normal = (vector.x / distance, vector.y / distance);
        direction = atan2(vector(2),vector(1)) * 180 / pi;
        if direction < 0
            direction = direction + 360; % Switch to positive degrees
        end
        % Set range of angles for falling objects
        fall = magnitude > 1 && (direction < 150) && (direction > 30);
    end
...
% to eliminate noisy detections, such as size, location, or appearance.
    ...
    function createNewTracks()
        ...
        for i = 1:size(centroids, 1)
            ...
            % Detect bottom center of bounding box.
            baseCenter = centroid;
            baseCenter(2) = centroid(2) + bbox(4)/2;
...
% displays the frame and the mask in their respective video players. 
    ...
    function displayTrackingResults()
        ...
        if ~isempty(tracks)
            ...
            seenTracks = reliableTracks(seenTrackInds);
            fallingTrackInds = ...
                [seenTracks(:).falling] == true;
            fallingTracks = seenTracks(fallingTrackInds);
            nonfallingTrackInds = ...
                [seenTracks(:).falling] == false;
            nonfallingTracks = seenTracks(nonfallingTrackInds);
            
            % Display the objects. If an object has not been detected
            % in this frame, display its predicted bounding box.
%             if ~isempty(seenTracks)
%                 % Get bounding boxes.
%                 bboxes = cat(1, seenTracks.bbox);
%                 
%                 % Get ids.
%                 ids = int32([seenTracks(:).normalizedDirection]);
%                 
...
%                     bboxes, labels,'Color','blue');
%             end
            draw_annotations(fallingTracks,'blue');
            draw_annotations(nonfallingTracks,'yellow');
        end
    ...
    function draw_annotations(group, color)
        if ~isempty(group)
            % Get bounding boxes.
            bboxes = cat(1, group.bbox);
